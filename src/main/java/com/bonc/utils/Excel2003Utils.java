...
public class Excel2003Utils {
    ...
    private static File exportExcelTitle(File file, String sheetName, List<String> columnNames,
        ...
        // 合并单元格
        sheet.addMergedRegion(new CellRangeAddress(lastRowIndex, lastRowIndex+2, 0, columnNames.size() - 1));
        // 产生表格标题行
    ...
    private static File exportExcelData(File file, String sheetName, List<List<Object>> objects) throws IOException {
        ...
        for (List<Object> dataRow : objects) {
            Row row = sheet.createRow(lastRowIndex);
            lastRowIndex++;
            sheet.addMergedRegion(new CellRangeAddress(lastRowIndex, lastRowIndex, dataRow.size() - 3, dataRow.size()-2));
